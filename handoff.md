...
> Documento de traspaso para retomar el trabajo rápido. Última actualización: 2026-06-05 (docs INDECOPI con coautores + estado de git).

---

> **Estado al 2026-06-05:** El código está **pusheado a GitHub** (`git log origin/main..HEAD` vacío; el botón "Reconectar voz" del commit `4ddd189` ya está en origin → el "pendiente: git push" de abajo ya se cumplió). Lo único **sin commitear** son los **documentos de INDECOPI** (regeneración con coautores: `Documento 0` índice, `Documento 7` cesión de derechos, `Formulario INDECOPI ... (rellenado).docx`, `Plantilla Registro General ... (rellenado).docx`, y Memoria/Manual/Plan/Docs 1–6 regenerados). Autoría en colaboración: titular único **Italo**, coautores **Jack J. Bendezu Lagos** y **Dickmar W. Julca Laureano** (ceden patrimoniales), profesor **Maglioni Arana Caparachin** solo asesor. Falta a mano: datos personales de los 2 coautores en el formulario + firmas en tinta. Expediente listo para presentar en la DDA de INDECOPI. **Pendientes del proyecto (no docs):** (1) cargar saldo en Anthropic — cuenta en $0, bloqueador #1; (2) conectar dominio `tuagentepro.xyz` a Railway; (3) al desplegar: `alembic upgrade head` (007/008) + recrear contenedores; (4) WhatsApp Business API oficial por cliente; (5) RLS nativo Postgres + bucket Supabase para backups off-site.

> **Botón "Reconectar voz" (2026-06-04):** NUEVO. El dueño borró por error el agente de Retell de su negocio; como cambiar de plan NO re-provisiona y no había forma de recrear el agente de un negocio existente (el candado `not tenant.retell_agent_id` lo bloqueaba), se agregó: (1) `TenantProvisioner.reprovision_voice(tenant, db)` — exige plan con voz, crea AgentConfig/VoiceConfig si faltan, compra número Twilio si no hay (o re-apunta el webhook de voz si ya hay), descarta el agente Retell viejo (best-effort) y crea uno nuevo, guarda `retell_agent_id` en tenant + voice_config; NO toca datos del negocio. (2) Helper `twilio_provisioner.configure_voice_webhook(number, slug)`. (3) Endpoint `POST /admin/tenants/{id}/provision-voice` (AdminGuard). (4) Botón "Reconectar voz" (ícono 📞 PhoneCall) en la fila del negocio en `AdminPage.tsx`. `ProvisioningError`→500 con `detail` claro (plan sin voz / Retell sin key). **21 tests admin verdes** (+2 nuevos: 404 y plan-sin-voz) + `npm run build` ✅. Documentado en `docs/GUIA_ENTERPRISE_PASO_A_PASO.md` §4b. ✅ **Ya pusheado** (commit `4ddd189` en origin); Railway redespliega solo.

> **Enterprise en vivo + guía total (2026-06-04):** Plataforma YA desplegada en Railway (`https://proyecto-agentepro-production.up.railway.app/`); falta conectar el dominio `tuagentepro.xyz` (CNAME a Railway; usar Cloudflare para la raíz `@`). El dueño quiere dejar su ÚNICO negocio como **Enterprise** real (número Twilio + voz Retell + automatizaciones) y probarlo. Se creó **`docs/GUIA_ENTERPRISE_PASO_A_PASO.md`** (exhaustiva): dominio → variables Railway → Twilio de pago → crear negocio Enterprise (auto-provisiona) → verificar → probar voz/WhatsApp → datos → costos → troubleshooting + checklist. **Hallazgos clave confirmados en código:** (1) el agente de Retell y el número Twilio se crean AUTOMÁTICO al provisionar un negocio con plan de voz (`tenant_provisioner.provision_new_tenant`), 1 agente por negocio; (2) **cambiar el plan NO re-provisiona** (no compra número ni crea agente) → para darle voz a un negocio existente, recrearlo o hacerlo manual; (3) el número y Retell se unen vía el backend: Twilio llama a `/webhooks/twilio/voice/{slug}` → TwiML hace `<Stream>` a `wss://api.retellai.com/audio-websocket/{retell_agent_id}`; (4) **`FRONTEND_URL` debe ser `https://tuagentepro.xyz` ANTES de provisionar**, porque `twilio_provisioner._backend_base()` deriva de ahí la URL del webhook que se graba en Twilio (default localhost rompe las llamadas); (5) WhatsApp NO se auto-provisiona (el número se configura solo para VOZ) → usar Sandbox para probar ya o registrar un WhatsApp Sender (1–3 días Meta). Twilio debe estar en modo PAGO para comprar número; Perú suele no tener números → cae a +1 US (sirve igual para probar). También se creó `docs/GUIA_CONECTAR_CLIENTE_NUEVO.md` (proceso por cada venta) con costos/márgenes.
